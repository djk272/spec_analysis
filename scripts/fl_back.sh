...
#
#
#this script grabs observation specific blanksky files for each chip, merges them and reprojects them to match to the observation coordinates/orientation
##########
...
do           
    export obsid=$line  
    echo $obsid
    dmcopy $obsid\/repro/*_evt2.fits[ccd_id=0:3] $obsid\/repro/evt2.fits clobber=yes	
	...
	if [ "$chips" = "5" ]; then
		...
		dmreadpar $obsid\/repro/event_pnt.par "$obsid\/repro/bkgrnd/merge_bg_clean.fits[events]" clobber+ 		
		...
		echo ${F:60:1}
		dmmerge "$A,$B,$C,$D,$E,$F" $obsid\/repro/bkgrnd/merge_bg.fits   
